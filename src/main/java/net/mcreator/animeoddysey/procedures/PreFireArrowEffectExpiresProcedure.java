...
public class PreFireArrowEffectExpiresProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof ServerLevel _level)
			_level.sendParticles((SimpleParticleType) (AnimeoddyseyModParticleTypes.FIRE_ARROW_PARTICLE.get()), x, y, z, 1, 0.1, 0.1, 0.1, 0);
		{
			...
			if (!projectileLevel.isClientSide()) {
				Projectile _entityToSpawn = new Object() {
					public Projectile getArrow(Level level, Entity shooter, float damage, int knockback) {
						AbstractArrow entityToSpawn = new FireArrowProjectileEntity(AnimeoddyseyModEntities.FIRE_ARROW_PROJECTILE.get(), level);
				...
				}.getArrow(projectileLevel, entity, (float) (30 + (entity.getCapability(AnimeoddyseyModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new AnimeoddyseyModVariables.PlayerVariables())).Moves / 3), 0);
