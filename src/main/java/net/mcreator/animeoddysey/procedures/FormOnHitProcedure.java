...
public class FormOnHitProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		double dodge = 0;
		if (((entity.getCapability(AnimeoddyseyModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new AnimeoddyseyModVariables.PlayerVariables())).Application).equals("Whirlwind") || entity instanceof KilluaZoldyckEntity) {
			if (entity instanceof LivingEntity _livEnt1 && _livEnt1.hasEffect(AnimeoddyseyModMobEffects.GODSPEED.get())) {
		...
		if (sourceentity instanceof LivingEntity _livEnt11 && _livEnt11.hasEffect(AnimeoddyseyModMobEffects.GEARSHIFT_OVERDRIVE.get())) {
			{
				...
				if (!projectileLevel.isClientSide()) {
					...
					}.getArrow(projectileLevel, entity, 0, 8);
